...
(function() {
  ...
  function drawContent(ctx, W, data, dateStr, dryRun) {
    ...
    var RADAR_H = 320;
    if(!dryRun){
      var radarCx=cx, radarCy=y+130, radarR=110, n7=data.catScores.length;
      var angleStep=(2*Math.PI)/n7, startAngle=-Math.PI/2;
      for(var g=1;g<=5;g++){var r=radarR*(g/5);ctx.beginPath();for(var i=0;i<=n7;i++){var a=startAngle+i*angleStep;var px=radarCx+r*Math.cos(a);var py=radarCy+r*Math.sin(a);if(i===0)ctx.moveTo(px,py);else ctx.lineTo(px,py);}ctx.closePath();ctx.strokeStyle='#e2e8f0';ctx.lineWidth=1;ctx.stroke();}
      for(var i=0;i<n7;i++){var a=startAngle+i*angleStep;ctx.beginPath();ctx.moveTo(radarCx,radarCy);ctx.lineTo(radarCx+radarR*Math.cos(a),radarCy+radarR*Math.sin(a));ctx.strokeStyle='#e2e8f0';ctx.lineWidth=1;ctx.stroke();}
      ctx.beginPath();for(var i=0;i<n7;i++){var a=startAngle+i*angleStep;var r=radarR*(data.catScores[i].pct/100);var px=radarCx+r*Math.cos(a);var py=radarCy+r*Math.sin(a);if(i===0)ctx.moveTo(px,py);else ctx.lineTo(px,py);}ctx.closePath();ctx.fillStyle='rgba(45,139,87,0.2)';ctx.fill();ctx.strokeStyle='#2D8B57';ctx.lineWidth=2;ctx.stroke();
      for(var i=0;i<n7;i++){var a=startAngle+i*angleStep;var r=radarR*(data.catScores[i].pct/100);ctx.beginPath();ctx.arc(radarCx+r*Math.cos(a),radarCy+r*Math.sin(a),4,0,Math.PI*2);ctx.fillStyle='#2D8B57';ctx.fill();}
      ctx.textAlign='center';var sn=["業務プロセス","Web活用","データ管理","データ活用","AI活用","セキュリティ","DX推進体制"];
      for(var i=0;i<n7;i++){var a=startAngle+i*angleStep;var lR=radarR+32;var lx=radarCx+lR*Math.cos(a);var ly=radarCy+lR*Math.sin(a);ctx.fillStyle='#333';ctx.font='bold 12px '+FONT;ctx.fillText(sn[i],lx,ly+4);ctx.fillStyle=getBarHex(data.catScores[i].pct);ctx.font='bold 11px '+FONT;ctx.fillText(data.catScores[i].pct+'点',lx,ly+18);}
      ctx.textAlign='left';
    }
    y+=RADAR_H;
