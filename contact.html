// ============================================================
// DX診断結果 PDF + 無料相談連携 v8
// PDF生成 → IndexedDB保存 → contact.html連携
// ============================================================
(function() {
  'use strict';
  ...
  var DB_NAME = 'stareight_dx';
  var STORE_NAME = 'pdf_store';
  var PDF_KEY = 'diagnosis_pdf';
  ...
  function rewriteCtaLink() {
    ...
    params.set('pdf', '1'); // PDFがIndexedDBにある印
    var contactUrl = 'contact.html?' + params.toString();
    var links = results.querySelectorAll('a[href="contact.html"]');
    links.forEach(function(a) { a.href = contactUrl; });
  }

  // ====== PDFを事前生成しIndexedDBに保存 ======
  function preGeneratePdf() {
    loadJsPdf(function(err) {
      if (err) return;
      try {
        var pdfBlob = buildPdfBlob();
        savePdfToIDB(pdfBlob);
      } catch (e) { console.warn('Pre-generate PDF failed:', e); }
    });
  }
  ...
  function drawContent(ctx, W, data, dateStr, dryRun) {
    ...
    if (!dryRun) {
      ...
      ctx.fillStyle='#fff'; ctx.font='bold 15px '+FONT; ctx.fillText('★ StarEight DX Consulting',PAD,35);
    ...
    data.catScores.forEach(function(c){
      ...
      if(!dryRun){ctx.fillStyle='#333';ctx.font='15px '+FONT;ctx.textAlign='left';ctx.fillText(c.icon+'  '+c.name,PAD,y);ctx.fillStyle=bc;ctx.font='bold 16px '+FONT;ctx.textAlign='right';ctx.fillText(c.pct+'点',W-PAD,y);ctx.textAlign='left';}
      y+=10;
      if(!dryRun){ctx.fillStyle='#eee';roundRect(ctx,PAD,y,CW,10,5);ctx.fill();if(c.pct>0){ctx.fillStyle=bc;roundRect(ctx,PAD,y,Math.max(10,(c.pct/100)*CW),10,5);ctx.fill();}}
      y+=30;
    });
    y+=20;
    ...
    for(var i=0;i<3&&i<data.sorted.length;i++){
      ...
      if(!dryRun){ctx.fillStyle='#f8fafb';roundRect(ctx,PAD,y,CW,cardH,8);ctx.fill();ctx.strokeStyle='#e0e8ef';ctx.lineWidth=1;roundRect(ctx,PAD,y,CW,cardH,8);ctx.stroke();ctx.fillStyle='#2980b9';ctx.fillRect(PAD,y+8,4,cardH-16);}
